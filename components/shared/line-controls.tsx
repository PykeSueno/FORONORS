...
export function QuantityStepper({ value, onDecrease, onIncrease, onChange, extraAction }: QuantityStepperProps) {
  return (
    <div className="flex h-9 items-center gap-1">
      ...
      <input
        className="saas-input !h-9 w-14 px-1 text-center text-sm"
        value={value}
        onChange={(event) => onChange(Number(event.target.value || 0))}
      />
      <button type="button" className="saas-ghost-btn !h-9 !min-h-9 !px-2 !py-0" onClick={onIncrease}>+</button>
      {extraAction ? <button type="button" className="saas-primary-btn !h-9 !min-h-9 !px-2 !py-0 text-[10px]" onClick={extraAction.onClick}>{extraAction.label}</button> : null}
    </div>
  );
}
