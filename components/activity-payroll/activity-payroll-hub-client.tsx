...
function firstAllowedTab(props: Pick<Props, 'canGlobal' | 'canActivities' | 'canPayroll' | 'canHistory' | 'canLogs'>): Tab {
  if (props.canGlobal) return 'global';
    ...
    <div className="space-y-4">
      ...
      <section className="glass-card p-3">
        <div className="flex flex-wrap gap-2">
          {props.canGlobal ? <TabButton active={tab === 'global'} onClick={() => setTab('global')}>👤 Vue globale</TabButton> : null}
      ...
      {tab === 'global' && props.canGlobal ? <GlobalView members={props.members} onDetail={showMember} /> : null}
      ...
      {tab === 'payroll' && props.canPayroll ? (
        <PayrollView adjustments={adjustments} canAdjust={props.canAdjust} canConfigure={props.canConfigure} canExclude={props.canExclude} canPay={props.canPay} config={config} customEnd={customEnd} customStart={customStart} effectivePreview={effectivePreview} excludedIds={excludedIds} loadPeriod={loadPeriod} paidMembers={paidMembers} paidTotal={paidTotal} payrollAction={payrollAction} periodMode={periodMode} previousPreview={previousPreview} saveConfig={saveConfig} saving={saving} selectedRange={selectedRange} setConfig={setConfig} setCustomEnd={setCustomEnd} setCustomStart={setCustomStart} updateAdjustment={(memberId, amount) => setAdjustments((cur) => ({ ...cur, [memberId]: Math.max(0, Math.round(amount)) }))} />
      ) : null}
...
function GlobalView({ members, onDetail }: { members: MemberSummary[]; onDetail: (memberId: string) => void }) {
  return (
    <section className="glass-card p-4">
      <div className="mb-3 flex items-center justify-between gap-2"><h2 className="text-base font-semibold text-[#fff1dd]">👤 Par membre</h2><span className="rounded-full border border-white/10 bg-[#3f281b]/60 px-2 py-1 text-[11px] text-[#efcdab]">{members.length} membres</span></div>
        ...
        {members.map((member) => (
          <article key={member.id} className="rounded-xl border border-white/10 bg-[#3f281b]/55 p-3">
            <div className="grid gap-3 sm:grid-cols-[1.2fr_repeat(4,minmax(0,1fr))_auto] sm:items-center">
              ...
              <Mini icon="🎯" label="Activites" value={String(member.activityCount)} />
              <Mini icon="💸" label="Paye" value={formatUsd(member.proposedPay)} />
              <Mini icon="🕒" label="Derniere" value={member.lastActivity ? new Date(member.lastActivity).toLocaleDateString('fr-FR') : '-'} />
...
function PayrollView(props: { canAdjust: boolean; canConfigure: boolean; canExclude: boolean; canPay: boolean; config: PayrollConfig; customEnd: string; customStart: string; effectivePreview: PayrollPreview; excludedIds: string[]; paidMembers: Record<string, number>; paidTotal: number; periodMode: PeriodMode; previousPreview: PayrollPreview; saving: boolean; selectedRange: { startIso: string; endIso: string }; setConfig: Dispatch<SetStateAction<PayrollConfig>>; setCustomEnd: (value: string) => void; setCustomStart: (value: string) => void; adjustments: Record<string, number>; updateAdjustment: (memberId: string, amount: number) => void; loadPeriod: (mode: PeriodMode, start?: string, end?: string) => Promise<void>; payrollAction: (action: 'pay' | 'adjust' | 'exclude', row: PayrollMemberRow, enabled?: boolean) => Promise<void>; saveConfig: () => Promise<void> }) {
  return (
    <div className="space-y-4">
      <section className="grid gap-2 lg:grid-cols-6"><Metric icon="💳" label="Argent groupe" value={formatUsd(props.effectivePreview.balance)} /><Metric icon="🛡️" label="Reserve" value={formatUsd(props.effectivePreview.reserveKept)} /><Metric icon="📦" label="Enveloppe" value={formatUsd(props.effectivePreview.envelope)} /><Metric icon="✅" label="Calcule" value={formatUsd(props.effectivePreview.totalProposed)} /><Metric icon="🏦" label="Apres paye" value={formatUsd(props.effectivePreview.balance - props.paidTotal)} /><Metric icon="👥" label="Eligibles" value={String(props.effectivePreview.eligibleCount)} /></section>
      <section className="glass-card p-4"><h3 className="text-sm font-semibold text-[#fff1dd]">📆 Periode de calcul</h3><div className="mt-2 flex flex-wrap gap-2"><button className={`filter-pill ${props.periodMode === 'current' ? 'filter-pill-active' : ''}`} onClick={() => void props.loadPeriod('current')}>Semaine actuelle</button><button className={`filter-pill ${props.periodMode === 'previous' ? 'filter-pill-active' : ''}`} onClick={() => void props.loadPeriod('previous')}>Semaine passee</button><button className={`filter-pill ${props.periodMode === 'custom' ? 'filter-pill-active' : ''}`} onClick={() => void props.loadPeriod('custom')}>Personnalisee</button></div>{props.periodMode === 'custom' ? <div className="mt-3 grid gap-2 md:grid-cols-[1fr_1fr_auto]"><input className="saas-input" type="datetime-local" value={props.customStart} onChange={(event) => props.setCustomStart(event.target.value)} /><input className="saas-input" type="datetime-local" value={props.customEnd} onChange={(event) => props.setCustomEnd(event.target.value)} /><button className="saas-primary-btn" onClick={() => void props.loadPeriod('custom', props.customStart, props.customEnd)}>Appliquer</button></div> : null}<p className="mt-2 text-xs text-[#efcdab]">Active: {props.selectedRange.startIso.slice(0, 10)} → {props.selectedRange.endIso.slice(0, 10)} · Exclusions: {props.excludedIds.length}</p></section>
      ...
      <section className="glass-card p-4"><h3 className="text-sm font-semibold text-[#fff1dd]">💸 Payes membres</h3><div className="mt-2 max-h-[560px] space-y-2 overflow-auto pr-1">{props.effectivePreview.members.map((member) => { const isPaid = Boolean(props.paidMembers[member.memberId]); const isExcluded = props.excludedIds.includes(member.memberId); return <article key={member.memberId} className="rounded-xl border border-white/10 bg-[#3f281b]/55 p-3 text-xs text-[#efcdab]"><div className="grid gap-2 xl:grid-cols-[1.2fr_repeat(5,minmax(0,1fr))_320px] xl:items-center"><div><p className="font-semibold text-[#ffe8ca]">{member.memberLabel}</p><p>{isPaid ? '✅ Paye' : isExcluded ? '🚫 Exclu' : member.eligible ? '💸 A payer' : member.reason}</p></div><Mini icon="💵" label="Argent rapporte" value={formatUsd(member.moneyContribution)} /><Mini icon="🎯" label="Actions" value={String(member.activityCount)} /><Mini icon="🤝" label="Implication" value={String(member.participationCount)} /><Mini icon="📊" label="Score" value={member.totalScore.toFixed(2)} /><Mini icon="💸" label="Paye" value={formatUsd(member.proposedPay)} /><div className="flex flex-wrap justify-end gap-2">{props.canAdjust ? <input className="saas-input !h-9 w-24" value={props.adjustments[member.memberId] ?? ''} placeholder="Ajuster" onChange={(event) => props.updateAdjustment(member.memberId, Number(event.target.value || 0))} /> : null}{props.canAdjust ? <button className="saas-ghost-btn !h-9 px-3" onClick={() => void props.payrollAction('adjust', member)}>🧮 Ajuster</button> : null}{props.canExclude ? <button className="saas-ghost-btn !h-9 px-3" onClick={() => void props.payrollAction('exclude', member, !isExcluded)}>{isExcluded ? '↩️ Reinclure' : '🚫 Exclure'}</button> : null}{props.canPay ? <button className="saas-primary-btn !h-9 px-3" disabled={isPaid || isExcluded || !member.eligible || member.proposedPay <= 0} onClick={() => void props.payrollAction('pay', member)}>{isPaid ? '✅ Paye' : '💸 Payer'}</button> : null}</div></div></article>; })}</div></section>
    </div>
  );
}
...
function Settings({ config, saving, setConfig, saveConfig }: { config: PayrollConfig; saving: boolean; setConfig: Dispatch<SetStateAction<PayrollConfig>>; saveConfig: () => Promise<void> }) {
  return <section className="glass-card space-y-3 p-4"><h3 className="text-sm font-semibold text-[#fff1dd]">⚙️ Reglages paye</h3><div className="grid gap-2 md:grid-cols-4"><Field label="Reserve minimale" tooltip={TOOLTIP.reserve} value={config.reserveMinimum} onChange={(v) => setConfig((cur) => ({ ...cur, reserveMinimum: v }))} /><Field label="% distribuable" tooltip={TOOLTIP.percent} value={Math.round(config.distributablePercent * 100)} onChange={(v) => setConfig((cur) => ({ ...cur, distributablePercent: Math.max(0, Math.min(100, v)) / 100 }))} /><Field label="Plafond membre" tooltip={TOOLTIP.cap} value={config.memberCap} onChange={(v) => setConfig((cur) => ({ ...cur, memberCap: v }))} /><Field label="Minimum membre" tooltip={TOOLTIP.minimum} value={config.memberMinimum} onChange={(v) => setConfig((cur) => ({ ...cur, memberMinimum: v }))} /></div><div className="grid gap-2 md:grid-cols-5"><Field label="Poids argent (%)" tooltip={TOOLTIP.money} value={Math.round(config.weights.money * 100)} onChange={(v) => setConfig((cur) => ({ ...cur, weights: { ...cur.weights, money: v / 100 } }))} /><Field label="Poids activite (%)" tooltip={TOOLTIP.activity} value={Math.round(config.weights.activity * 100)} onChange={(v) => setConfig((cur) => ({ ...cur, weights: { ...cur.weights, activity: v / 100 } }))} /><Field label="Poids implication (%)" tooltip={TOOLTIP.participation} value={Math.round(config.weights.participation * 100)} onChange={(v) => setConfig((cur) => ({ ...cur, weights: { ...cur.weights, participation: v / 100 } }))} /><Field label="Seuil actions" tooltip={TOOLTIP.actions} value={config.minActions} onChange={(v) => setConfig((cur) => ({ ...cur, minActions: v }))} /><Field label="Seuil argent" tooltip={TOOLTIP.minMoney} value={config.minMoney} onChange={(v) => setConfig((cur) => ({ ...cur, minMoney: v }))} /></div><button className="saas-primary-btn" disabled={saving} onClick={() => void saveConfig()}>{saving ? 'Enregistrement...' : '💾 Enregistrer les reglages'}</button></section>;
}

function HistoryView({ rows }: { rows: HistoryPayment[] }) {
  return <section className="glass-card p-4"><h2 className="mb-3 text-base font-semibold text-[#fff1dd]">📜 Historique des payes</h2><div className="max-h-[620px] space-y-2 overflow-auto pr-1">{rows.map((row) => <article key={row.id} className="rounded-xl border border-white/10 bg-[#3f281b]/55 p-3 text-xs text-[#efcdab]"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold text-[#ffe8ca]">{row.member_label} · {formatUsd(Number(row.amount ?? 0))}</p><p>{new Date(row.created_at).toLocaleString('fr-FR')}</p></div><p>Periode: {row.week_start.slice(0, 10)} → {row.week_end.slice(0, 10)} · Solde: {formatUsd(Number(row.group_balance_before ?? 0))} → {formatUsd(Number(row.group_balance_after ?? 0))}</p></article>)}{rows.length === 0 ? <p className="text-xs text-[#efcdab]">Aucune paye versee.</p> : null}</div></section>;
}
