  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        else if (response.status === 403) setError('Compte inactif ou acces refuse.');
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden px-4">
      ...
      <div className="glass-card relative z-10 w-full max-w-md p-8">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <input className="saas-input w-full" placeholder="Username" value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" required />
          <input
            type="password"
