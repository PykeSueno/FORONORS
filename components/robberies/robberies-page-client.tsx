...
export function RobberiesPageClient({ runs, items, members, canCreate, canArrested, canStats, canLogs, stats, playerStats }: {
  ...
  const rotationSuggestions = useMemo<Suggestion[]>(() => [
    { title: 'Braqueurs conseillés', icon: '🎯', names: suggest(roleStats, 'braqueur') },
    { title: 'Mule / récup conseillés', icon: '🚗', names: suggest(roleStats, 'plan_mule_recup') },
    { title: 'Otages conseillés', icon: '🤝', names: suggest(roleStats, 'otage_apporte') }
  ], [roleStats]);
