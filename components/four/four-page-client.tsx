    ...
    <div className="space-y-4">
      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_1.1fr]">
        ...
        <article className="glass-card p-5 space-y-3">
          ...
          <input className="saas-input w-full" placeholder="Contrepartie (optionnel)" value={counterparty} onChange={(e) => setCounterparty(e.target.value)} />
          {draftLines.map((line, idx) => (
            <div key={`${line.item_id}-${idx}`} className="rounded-xl border border-white/10 bg-[#2f1d14]/45 p-2">
              ...
              <div className="mt-2 grid gap-2 md:grid-cols-4">
                ...
                <input className="saas-input" value={line.quantity} onChange={(e) => setDraftLines((cur) => cur.map((entry, i) => i === idx ? { ...entry, quantity: Math.max(1, Number(e.target.value || 1)) } : entry))} />
          ...
          <div className="rounded-xl border border-white/10 bg-[#3b2418]/55 p-3 text-sm">
            <p>Achats: {formatUsd(draftTotals.purchases)} · Ventes: {formatUsd(draftTotals.sales)} · Résultat: {formatUsd(draftTotals.profit)}</p>
          </div>
