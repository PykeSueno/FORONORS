...
export function DashboardShellClient({ name, role, payEstimateCurrent, payEstimatePrevious, canUpdatePassword, initialOrder, flags }: { name: string; role: string; payEstimateCurrent: number; payEstimatePrevious: number; canUpdatePassword: boolean; initialOrder: string[]; flags: DashboardFlags }) {
  ...
  const cards = useMemo<Card[]>(() => [
    ...
    flags.canTabletCigarettePreview ? { id: 'tablet_cigarette', href: '/dashboard/tablette-cigarette', enabled: flags.canTabletCigaretteAccess, icon: '📱🚬', title: 'Tablette & Tabac', value: summary ? String(summary.values.tabletPassagesToday + summary.values.cigarettePassagesToday) : '…', subtitle: summary ? `Tablette / Tabac / Processeur (${formatUsd(summary.values.cigaretteRevenueToday)})` : 'Tablette / Tabac / Processeur' } : null,
