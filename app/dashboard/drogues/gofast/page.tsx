...
type MemberRow = { id: string; name: string | null; username: string | null };
...
export default async function DrugsGoFastPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <GoFastPageClient
        items={filteredItems}
        members={((members ?? []) as MemberRow[]).map((member) => ({ id: member.id, label: member.name || member.username || 'Membre' }))}
        runs={runs ?? []}
        stats={stats}
        canCreate={canCreate}
