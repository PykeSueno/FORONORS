...
type ParticipantUser = { id: string; name: string | null; username: string | null };
...
export async function POST(request: Request) {
  ...
  const participantIds = Array.from(new Set((body.participant_ids ?? []).map((entry) => entry.trim()).filter(Boolean)));
  ...
  const participantRows = ((participants ?? []) as ParticipantUser[]).map((entry) => ({ id: entry.id, label: entry.name || entry.username || 'Membre' }));
  if (participantRows.length === 0) return NextResponse.json({ message: 'Participants introuvables.' }, { status: 400 });
  ...
  await createAuditLog({
    ...
    summary: `${action === 'success' ? 'GoFast réussi' : 'GoFast arrêté'} — ${item.name} x${quantity} — ${participantRows.map((entry) => entry.label).join(', ')}`,
    newValues: {
      itemId: item.id,
      itemName: item.name,
      quantity,
