...
export async function PATCH(request: Request) {
  ...
  if (!day) {
    ...
    await supabase.from('tablet_days').update({ deposited_amount: deposit, chest_amount: deposit, updated_at: new Date().toISOString() }).eq('id', day.id);
  }
  ...
  await createAuditLog({
    ...
    summary: `Dépôt coffre tablette du jour: ${deposit}$`,
    newValues: { businessDay, deposit }
  });
