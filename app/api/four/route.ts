...
export async function PATCH(request: Request) {
  ...
  const lockPayload = {
    ...
    closing_started_by: access.session.userId
  };
  const { data: lockRows } = await supabase
    .from('four_sessions')
    .update({ status: 'closing', summary: lockPayload })
    .eq('id', sessionId)
    .eq('status', 'open')
    .select('id');
  if (!lockRows || lockRows.length === 0) return NextResponse.json({ message: 'Clôture déjà en cours ou session non ouverte.' }, { status: 409 });

  try {
    const itemDelta = new Map<number, number>();
    let totalPurchases = 0;
    let totalSales = 0;
    const validatedTransactions = (session.four_transactions ?? []).filter((tx) => (tx.status ?? 'validated') === 'validated');
    ...
    for (const [itemId, delta] of itemDelta.entries()) {
      ...
      if (next < 0) throw new Error(`Stock insuffisant pour l'item #${itemId}.`);
    ...
    if (nextBalance < 0) throw new Error('Solde insuffisant pour clôturer FOUR.');
    ...
    await supabase.from('four_sessions').update({
      ...
      summary: {
        ...(session.summary as Record<string, unknown> | null),
        closing_started_at: lockPayload.closing_started_at,
        closing_started_by: lockPayload.closing_started_by,
    ...
    await createAuditLog({
      ...
      newValues: { initialCash, cashAdded, totalPurchases, totalSales, profit: cashDelta, cashFinal, itemDelta: Object.fromEntries(itemDelta), validatedTxCount: validatedTransactions.length }
    });
    ...
    await supabase
      ...
      .update({
        summary: {
          ...(session.summary as Record<string, unknown> | null),
          closing_error_at: new Date().toISOString(),
          closing_error_message: error instanceof Error ? error.message : 'Erreur inconnue'
        }
      })
      .eq('id', sessionId)
      .eq('status', 'closing');
    return NextResponse.json({ message: error instanceof Error ? error.message : 'Clôture impossible.' }, { status: 400 });
  }
}
