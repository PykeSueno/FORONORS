...
import { CIGARETTE_ITEM_NAME, CIGARETTE_REVENUE, CIGARETTE_SALE_QTY, getCigaretteBusinessDate, isCigarettePassageHourAllowed } from '@/lib/cigarette';
...
export async function POST(request: Request) {
  ...
  await createAuditLog({
    ...
    summary: `Passage cigarette ${memberLabel} | packs ${beforePacks}->${afterPacks} | dépôt ${beforeChest}$->${afterChest}$ | groupe ${beforeGroupCash}$->${afterGroupCash}$`,
    oldValues: { beforePacks, beforeChest, beforeGroupCash },
    newValues: { businessDay, memberUserId: requestedMemberId, memberLabel, afterPacks, afterChest, afterGroupCash, quantitySold: CIGARETTE_SALE_QTY, revenueAmount: CIGARETTE_REVENUE, dayId: day.id }
  });
