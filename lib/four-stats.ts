...
export async function buildFourStats(): Promise<FourStatsPayload> {
  ...
  const [{ data: txRows }, { data: movementRows }] = await Promise.all([
    ...
    supabase
      .from('four_movements')
      .select('session_id, item_id, item_name, movement_kind, quantity, unit_price, total_amount, created_at, items(image_url)')
      .order('created_at', { ascending: false })
      .limit(4000)
  ]);
  ...
  for (const tx of transactions) {
    ...
    for (const line of tx.four_transaction_lines ?? []) {
      ...
      const current = byItemMap.get(itemId) ?? { itemId, itemName: line.item_name, imageUrl: itemPayload?.image_url ?? null, buyQty: 0, sellQty: 0, buyAmount: 0, sellAmount: 0, frequency: 0 };
      current.frequency += 1;
